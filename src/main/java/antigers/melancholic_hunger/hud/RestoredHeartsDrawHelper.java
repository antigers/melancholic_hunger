...
public class RestoredHeartsDrawHelper {
    public record RenderedHeart(@Nullable ResourceLocation texture, int guiAtlasPosition, Color color) {}

    public static final ResourceLocation WHITE_FULL_HEART_TEXTURE = new ResourceLocation(
            MelancholicHunger.MOD_ID, "white_full_heart_texture"
    );
    public static final ResourceLocation WHITE_HALF_HEART_TEXTURE = new ResourceLocation(
            MelancholicHunger.MOD_ID, "white_half_heart_texture"
    );
    public static final ResourceLocation WHITE_RIGHT_HALF_HEART_TEXTURE = new ResourceLocation(
            MelancholicHunger.MOD_ID, "white_right_half_heart_texture"
    );
    public static final ResourceLocation ORIGINAL_RIGHT_HALF_HEART_TEXTURE = new ResourceLocation(
            MelancholicHunger.MOD_ID, "original_right_half_heart_texture"
    );
    public static final ResourceLocation BLINKING_RIGHT_HALF_HEART_TEXTURE = new ResourceLocation(
        ...
        if (highlightRestoredHearts) {
            ...
            switch (MelancholicConfig.restoredHeartsTexture()) {
                case SINGLE_COLOR -> {
                    ...
                    restoredRightHalfHeartGetter = () -> new RenderedHeart(WHITE_RIGHT_HALF_HEART_TEXTURE, 0, restoredHeartsOverlayColor);
                }
                case ORIGINAL -> {
                    restoredHeartGetter = isHalf -> getAtlasSpriteHeart(Gui.HeartType.NORMAL, isHalf, false, restoredHeartsOverlayColor);
                    restoredRightHalfHeartGetter = () -> new RenderedHeart(ORIGINAL_RIGHT_HALF_HEART_TEXTURE, 0, restoredHeartsOverlayColor);
                }
                case BLINKING -> {
                    restoredHeartGetter = isHalf -> getAtlasSpriteHeart(Gui.HeartType.NORMAL, isHalf, true, restoredHeartsOverlayColor);
                    restoredRightHalfHeartGetter = () -> new RenderedHeart(BLINKING_RIGHT_HALF_HEART_TEXTURE, 0, restoredHeartsOverlayColor);
                }
            }
        }
    ...
    private RenderedHeart getWhiteHeart(boolean isHalf, Color color) {
        return new RenderedHeart(isHalf ? WHITE_HALF_HEART_TEXTURE : WHITE_FULL_HEART_TEXTURE, 0, color);
    }

    private RenderedHeart getAtlasSpriteHeart(Gui.HeartType type, boolean isHalf, boolean isBlinking, Color color) {
        return new RenderedHeart(null, type.getX(isHalf, isBlinking), color);
    }
