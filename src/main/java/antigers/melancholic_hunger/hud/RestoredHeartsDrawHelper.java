...
public class RestoredHeartsDrawHelper {
    ...
    public static final Identifier WHITE_HALF_HEART_TEXTURE = Identifier.fromNamespaceAndPath(
            MelancholicHunger.MOD_ID, "white_half_heart_texture"
    );
    public static final Identifier WHITE_RIGHT_HALF_HEART_TEXTURE = Identifier.fromNamespaceAndPath(
            MelancholicHunger.MOD_ID, "white_right_half_heart_texture"
    );
    public static final Identifier ORIGINAL_RIGHT_HALF_HEART_TEXTURE = Identifier.fromNamespaceAndPath(
            MelancholicHunger.MOD_ID, "original_right_half_heart_texture"
    );
    public static final Identifier BLINKING_RIGHT_HALF_HEART_TEXTURE = Identifier.fromNamespaceAndPath(
    ...
    public RestoredHeartsDrawHelper(Player player, RandomSource random) {
        ...
        if (highlightRestoredHearts) {
            ...
            switch (MelancholicConfig.restoredHeartsTexture()) {
                ...
                case ORIGINAL -> {
                    restoredHeartGetter = isHalf -> getAtlasSpriteHeart(Hud.HeartType.NORMAL, isHalf, false, restoredHeartsOverlayColor);
                    restoredRightHalfHeartGetter = () -> new RenderedHeart(ORIGINAL_RIGHT_HALF_HEART_TEXTURE, false, restoredHeartsOverlayColor);
                }
                case BLINKING -> {
                    restoredHeartGetter = isHalf -> getAtlasSpriteHeart(Hud.HeartType.NORMAL, isHalf, true, restoredHeartsOverlayColor);
                    restoredRightHalfHeartGetter = () -> new RenderedHeart(BLINKING_RIGHT_HALF_HEART_TEXTURE, false, restoredHeartsOverlayColor);
                }
            }
        }

        consumedNutrition = disableHunger ? HealthRegenerationComponent.getConsumedNutrition(player) : 0;
        heldFoodNutrition = foodComponent != null ? MelancholicConfig.getFoodValues(heldItemStack, foodComponent, player.level()).nutrition() : 0;
    ...
    private RenderedHeart getAtlasSpriteHeart(Hud.HeartType type, boolean isHalf, boolean isBlinking, Color color) {
